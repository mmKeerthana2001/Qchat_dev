...
class Agent:
    ...
    async def correct_query(self, query: str, history: list, role: str) -> str:
        try:
            ...
            # Skip correction if query is already clean and matches common patterns
            if query_lower.startswith(("what is", "what's", "how to", "can you")) and len(query.split()) > 2:
                logger.info(f"Skipping correction for clean query: '{query}'")
                return query
            ...
            prompt = (
                ...
                "correct any spelling, typing, or grammatical errors in the query. "
                "Infer the most likely intended meaning, ensuring the query is clear and natural. "
                "The query may relate to Quadrant Technologies locations or nearby amenities. "
    ...
    async def expand_query_for_retrieval(self, query: str, history: list, role: str) -> str:
        try:
            prompt = (
                "You are an expert at enhancing queries for semantic search in a document retrieval system. "
                f"You are interacting with a {'HR representative' if role == 'hr' else 'job candidate'}. "
                "Given the user query and conversation history, generate an expanded version of the query that includes synonyms, related terms, and rephrased variations to improve retrieval of relevant document chunks. "
                "The expanded query should capture the core intent and context of the original query, including terms relevant to HR policies, accommodations, or workplace procedures. "
                "Output ONLY the expanded query as a single string, without additional explanations."
            )
            prompt += f"\n\nConversation History:\n"
            for msg in history[-5:]:
                prompt += f"{msg['role'].capitalize()}: {msg['query']}\nAssistant: {msg['response']}\n"
            ...
            response = await self.client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "You are a query expansion assistant for semantic search."},
                    {"role": "user", "content": prompt}
                ],
                max_tokens=150,
                temperature=0.5
    ...
    async def process_query(self, documents: str, history: list, query: str, role: str, intent_data: dict = None) -> Tuple[str, dict | None]:
        try:
            ...
            # Build LLM prompt for document-based or other intents
            prompt = (
                "You are an expert assistant analyzing documents, designed to maintain conversation context like a chat application. "
                f"You are interacting with a {'HR representative' if role == 'hr' else 'job candidate'}. "
                "Below is the extracted text from relevant document sections and the conversation history. "
                "Answer the user's query by extracting specific paragraphs or lines from the provided documents that directly address the query. "
                "If the query is phrased differently but contextually related to the document content (e.g., asking about 'accommodation process' instead of 'accommodation requests'), identify and extract the most relevant information. "
                "Provide a concise and accurate response, quoting or summarizing the relevant document content when possible. "
                "If no relevant information is found in the documents, clearly state: 'I'm sorry, I couldn't find specific information in the provided documents to answer your query. Please rephrase your question or ensure relevant documents are uploaded.' "
            ...
            response = await self.client.chat.completions.create(
                ...
                max_tokens=600,  # Increased to handle detailed document-based responses
                temperature=0.4  # Lowered for precise, document-focused answers
            )
