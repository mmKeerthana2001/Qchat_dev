    ...
    if (lastMessage?.map_data && mapRef.current && window.google?.maps) {
      if (lastMessage.map_data.type === "nearby" && lastMessage.map_data.coordinates) {
        ...
        const map = new window.google.maps.Map(mapRef.current, {
          ...
          zoomControl: true,
          mapTypeControl: true, // Adds satellite/terrain/map toggle
          streetViewControl: true, // Adds Street View pegman
          fullscreenControl: true, // Adds fullscreen option
          styles: [ // Optional: Mimic Google Maps' clean style
            {
              featureType: "poi",
              elementType: "labels",
          ...
          const infoWindow = new window.google.maps.InfoWindow({
            content: `
              <div style="font-family: Roboto, Arial, sans-serif; padding: 8px;">
                <h3 style="font-size: 14px; font-weight: 500; margin: 0 0 4px;">${coord.label}</h3>
                ${lastMessage.map_data.data[index]?.rating ? `<p style="font-size: 12px; color: #5f6368;">Rating: ${lastMessage.map_data.data[index].rating} (${lastMessage.map_data.data[index].total_reviews} reviews)</p>` : ''}
                ${lastMessage.map_data.data[index]?.type ? `<p style="font-size: 12px; color: #5f6368;">Type: ${lastMessage.map_data.data[index].type}</p>` : ''}
              </div>
            `,
          });
      ...
      coordinates.forEach((coord, index) => {
        ...
        const infoWindow = new window.google.maps.InfoWindow({
          content: `
            <div style="font-family: Roboto, Arial, sans-serif; padding: 8px;">
              <h3 style="font-size: 14px; font-weight: 500; margin: 0 0 4px;">${coord.label}</h3>
              <p style="font-size: 12px; color: #5f6368;">${index === 0 ? 'Origin' : 'Destination'}</p>
            </div>
          `,
        });
      ...
      const polyline = new window.google.maps.Polyline({
        ...
        geodesic: true,
        strokeColor: '#1A73E8', // Google Maps' default blue
        strokeOpacity: 1.0, // Full opacity for bold look
        strokeWeight: 8, // Thicker for prominence
        map: map,
